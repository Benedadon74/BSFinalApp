...
class ShippingTableViewController: UITableViewController,UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    //**Begin Copy**
    //2) Add filter search vars
    
    var filteredTableData = [NSManagedObject]()
    var resultSearchController = UISearchController()
    var shippingArray = [NSManagedObject]()
    ...
    func updateSearchResults(for searchController: UISearchController)
    {
        ...
        let array = (shippingArray as NSArray).filtered(using: searchPredicate)
    ...
    func loaddb()
    {
        ...
        let fetchRequest:NSFetchRequest<NSFetchRequestResult> = NSFetchRequest(entityName: "Shipping")
        
        do {
            let fetchedResults = try managedContext.fetch(fetchRequest) as? [NSManagedObject]
            if let results = fetchedResults {
                shippingArray = results
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (self.resultSearchController.isActive) {
            ...
            let cell =
                tableView.dequeueReusableCell(withIdentifier: "Cell")
                    as UITableViewCell?
            let person = shippingArray[(indexPath as NSIndexPath).row]
            cell?.textLabel?.text = person.value(forKey: "fullname") as! String?
            cell?.detailTextLabel?.text = ">>"
            return cell!
        }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            if (self.resultSearchController.isActive) {
                ...
                context.delete(shippingArray[(indexPath as NSIndexPath).row])
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "UpdateShipping" {
            if let destination = segue.destination as?
                ...
                if (self.resultSearchController.isActive) {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        ...
                        destination.shippingdb = selectedDevice
                        resultSearchController.isActive = false
                    }
                }
                else {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        let selectedDevice:NSManagedObject = shippingArray[SelectIndex] as NSManagedObject
                        destination.shippingdb = selectedDevice
                    }
                }
            }
