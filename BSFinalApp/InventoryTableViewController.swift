...
class InventoryTableViewController: UITableViewController,UISearchResultsUpdating, UISearchBarDelegate, UISearchControllerDelegate {
    //**Begin Copy**
    //2) Add filter search vars
    
    var filteredTableData = [NSManagedObject]()
    var resultSearchController = UISearchController()
    var inventoryArray = [NSManagedObject]()
    ...
    func updateSearchResults(for searchController: UISearchController)
    {
        ...
        let searchPredicate = NSPredicate(format: "itemName CONTAINS[c] %@", searchController.searchBar.text!)
        let array = (inventoryArray as NSArray).filtered(using: searchPredicate)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (self.resultSearchController.isActive) {
            ...
            cell?.textLabel?.text = person.value(forKey: "itemName") as! String?
            cell?.detailTextLabel?.text = person.value(forKey: "quantity") as! String?
            return cell!
        }
        else {
            let cell =
               tableView.dequeueReusableCell(withIdentifier: "Cell")
                   as UITableViewCell?
           let person = inventoryArray[(indexPath as NSIndexPath).row]
           cell?.textLabel?.text = person.value(forKey: "itemName") as! String?
           cell?.detailTextLabel?.text = person.value(forKey: "quantity") as! String?
           return cell!
        }
        //**End Copy** cell?.detailTextLabel?.text = ">>"
   }
    //**Begin Copy**
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        if editingStyle == .delete {
            ...
            if (self.resultSearchController.isActive) {
                ...
                context.delete(inventoryArray[(indexPath as NSIndexPath).row])
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "UpdateInventory" {
            if let destination = segue.destination as?
                ...
                if (self.resultSearchController.isActive) {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                       ...
                       destination.inventorydb = selectedDevice
                        resultSearchController.isActive = false
                    }
                }
                else {
                    if let SelectIndex = (tableView.indexPathForSelectedRow as NSIndexPath?)?.row {
                        let selectedDevice:NSManagedObject = inventoryArray[SelectIndex] as NSManagedObject
                        destination.inventorydb = selectedDevice
                    }
                }
            }
